...
class AStarPlanner:
    ...
    def __init__(self, ox, oy, resolution, rr, fc_x, fc_y, tc_x, tc_y, p_x, p_y):
        """
        ...
        while 1:
            ...
            for i, _ in enumerate(self.motion): # tranverse the motion matrix
                ...
                if self.calc_grid_position(node.x, self.min_x) in self.fc_x:
                  ...
                  # add more cost in fuel-consuming area
                if self.calc_grid_position(node.x, self.min_x) in self.p_x:
                    if self.calc_grid_position(node.y, self.min_y) in self.p_y:
...
def main():
    ...
    a_star = AStarPlanner(ox, oy, grid_size, robot_radius, fc_x, fc_y, tc_x, tc_y, p_x, p_y)
